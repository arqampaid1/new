...
TOKEN = '7905599697:AAGFHMtLycEdUzy6rzF3PkBPGhrk9VWXGc4'
MONGO_URI = 'mongodb+srv://arqamddos:sopore45@arqamddos.ihfyf.mongodb.net/?retryWrites=true&w=majority&appName=arqamddos'
FORWARD_CHANNEL_ID = -1002274374300
CHANNEL_ID = -1002274374300
error_channel_id = -1002274374300
...
ADMIN_ID = "5177094156"  # Example admin user ID
...
def create_inline_keyboard():
    ...
    button = InlineKeyboardButton(text="OWNER", url="https://t.me/mahikg0")
    keyboard.add(button)
    return keyboard
...
def list_approved_users(message):
    ...
    if message.from_user.id != 5177094156:
        bot.reply_to(message, "🚫 *Access Denied!*\n"
            "❌ *You don't have the required permissions to use this command.*\n"
            "💬 *Please contact the bot owner if you believe this is a mistake.*")
        return
...
def info_command(message):
    ...
    if user_data:
        ...
        admin_id = "5177094156"  # Hard-coded admin ID
...
/clearlogs : Clear The Logs File.
    ...
    keyboard = InlineKeyboardMarkup()
    button = InlineKeyboardButton(text="Owner", url="https://t.me/mahikg0")  # Owner button URL
    keyboard.add(button)
...
def owner_command(message):
    bot.send_message(message.chat.id, "*Owner - @mahikg0*", parse_mode='Markdown', reply_markup=create_inline_keyboard())
